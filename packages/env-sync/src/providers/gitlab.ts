 ...
 */
export function readGitLabConfig(): GitLabConfig {
    const projectId = process.env['CI_PROJECT_ID'];
    const token     = process.env['GITLAB_TOKEN'];
    const apiUrl    = process.env['CI_API_V4_URL'] ?? 'https://gitlab.com/api/v4';

    if (!projectId) throw new Error('CI_PROJECT_ID is required for GitLab provider');
    if (!token)     throw new Error('GITLAB_TOKEN is required for GitLab provider');
 ...
 */
async function fetchExistingKeys(config: GitLabConfig): Promise<ReadonlySet<string>> {
    ...
    const variables = (await response.json()) as ReadonlyArray<GitLabVariable>;
 ...
 */
export class GitLabProvider implements SecretProvider {
