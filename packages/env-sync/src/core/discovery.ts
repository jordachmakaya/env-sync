 ...
 */
function targetFileNames(environment: string): ReadonlySet<string> {
 ...
 */
function walk(dir: string, targets: ReadonlySet<string>, results: string[]): void {
    let entries: Dirent[];
 ...
 *   .env                       → "root"
 */
export function extractPackageName(filePath: string, root: string): string {
 ...
 */
export function discoverEnvFiles(root: string, environment: string): ReadonlyArray<string> {
    ...
    return results.sort(
        (a, b) => computePriority(a, environment) - computePriority(b, environment)
    );
}
