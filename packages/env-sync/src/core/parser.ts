...
import type { EnvFile, Provider } from '../types.js';
import { ENV_SYNC_PROVIDER_KEY } from '../types.js';

const VALID_PROVIDERS = new Set<Provider>(['gitlab', 'github']);
 ...
 */
function extractProviderOverride(values: Record<string, string>): Provider | null {
    const raw = values[ENV_SYNC_PROVIDER_KEY];
    if (!raw) return null;

    const candidate = raw.toLowerCase() as Provider;
