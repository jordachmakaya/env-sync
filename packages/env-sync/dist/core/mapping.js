 ...
 */
function namespaceKey(packageName, key) {
    if (packageName === 'root')
        return key;
 ...
 */
export function buildSecretMap(envFiles) {
    const map = new Map();
    for (const file of envFiles) {
        for (const [rawKey, value] of Object.entries(file.values)) {
            const key = namespaceKey(file.packageName, rawKey);
            map.set(key, {
                key,
                value,
                source: file.path,
 ...
 * 3. Default: 'gitlab'
 */
export function resolveProvider(envFiles, cliProvider) {
    if (cliProvider === 'gitlab' || cliProvider === 'github')
        return cliProvider;
