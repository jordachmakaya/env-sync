 ...
 */
export function defaultWorkflowDir(root) {
...
function discoverWorkflowFiles(dir) {
    ...
    return readdirSync(dir, { withFileTypes: true })
        .filter((e) => e.isFile() && WORKFLOW_EXTENSIONS.has(e.name.slice(e.name.lastIndexOf('.'))))
 ...
 */
function detectEnvIndent(lines) {
 ...
 */
function patchWorkflowContent(content, secretKeys, existingKeys) {
    const missingKeys = [...secretKeys].filter((k) => !existingKeys.has(k));
    if (missingKeys.length === 0)
        return { patched: content, added: [] };
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (inEnvBlock) {
            ...
            const isLastEnvLine = nextLine.trim() === '' ||
 ...
 */
export function patchWorkflows(workflowDir, secrets, dryRun) {
